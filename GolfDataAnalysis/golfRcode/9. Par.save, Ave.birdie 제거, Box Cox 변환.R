...
c = boxcox( Ave.Stroke~ Ave.putting+GIR+Par.break+Recovery.rate,data=df)
lamda = c$x[which(c$y==max(c$y))]
m2 <- lm(Ave.Stroke^(lamda)~ Ave.putting+GIR+Par.break+Recovery.rate,data=df)
summary(m2) #Par.save,Ave.birdie제거후 Box cox변환
require(car)
vif(m2)>10 #다중공선성검사
